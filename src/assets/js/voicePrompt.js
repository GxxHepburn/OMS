async function voicePrompt (text) {
  var token = ''
  var getTokenUrl = 'https://www.donghuastar.com/bttsToken'
  ...
  if (timeInstant != null && timeInstant > Date.parse(new Date())) {
    ...
    const { data: res } = await window.VueThat.$http.post(getTokenUrl, { tokenMy: window.sessionStorage.getItem('token') })
